...
def render_input_date(df, start_year, end_year):
    ...
    country_code = st.selectbox(
        "Select Country", countries, index=countries.index("ID"))

    holiday_file = os.path.join(
        holiday_folder, f'holidays_{country_code}.xlsx')
    ...
    if os.path.exists(holiday_file):
        ...
        df_holiday["Holiday Name"] = df_holiday.apply(
            lambda row: "Weekend" if row["Holiday Name"] == "" and row["Date"].weekday() in [
                5, 6] else row["Holiday Name"],
            axis=1
        )

        df_holiday["Libur"] = df_holiday["Holiday Name"].apply(
    ...
    if selected_date_ts in df_holiday["Date"].values:
        existing_name = df_holiday.loc[df_holiday["Date"]
                                       == selected_date_ts, "Holiday Name"].values[0]
    else:
        existing_name = ""

    holiday_name = st.text_input(
        "Enter the holiday name (leave empty to delete)", value=existing_name)

    col1, col2 = st.columns([4, 6])

    with col1:
        if st.button("Save Holiday Name"):
            selected_date_ts = pd.to_datetime(selected_date)
            df_holiday["Date"] = pd.to_datetime(df_holiday["Date"])

            if selected_date_ts in df_holiday["Date"].values:
                if holiday_name.strip() == "":
                    df_holiday.loc[df_holiday["Date"] == selected_date_ts, [
                        "Holiday Name", "Libur"]] = ["", 0]
                    st.warning(f"Holiday name deleted: {selected_date}")
                else:
                    df_holiday.loc[df_holiday["Date"] ==
                                   selected_date_ts, "Holiday Name"] = holiday_name
                    df_holiday.loc[df_holiday["Date"] ==
                                   selected_date_ts, "Libur"] = 1
                    st.success(
                        f"Holiday name altered: {selected_date} - {holiday_name}")
            else:
                if holiday_name.strip() != "":
                    new_row = pd.DataFrame(
                        {"Date": [selected_date_ts], "Holiday Name": [holiday_name], "Libur": [1]})
                    df_holiday = pd.concat(
                        [df_holiday, new_row], ignore_index=True)
                    st.success(
                        f"Holiday name added: {selected_date} - {holiday_name}")
                else:
                    st.info(
                        "There is no change because the holiday name is empty and the date was not found."
                    )
    ...
    with col2:
        if st.button("Update Holidays Until Today"):
            ...
            st.success("The holiday data has been successfully updated until today.")
